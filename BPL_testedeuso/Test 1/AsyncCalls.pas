...
function AsyncCallI(Proc: TAsyncCallArgInterfaceProc; const Arg: IInterface): IAsyncCall; overload;
...
function AsyncCallI(Proc: TAsyncCallArgInterfaceProc; const Arg: IInterface): IAsyncCall;
begin
  { Execute the function synchron if no thread pool exists }
  if ThreadPool.MaxThreads = 0 then
    Result := TSyncCall.Create(Proc(Arg))
